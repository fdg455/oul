...
import com.oushangfeng.ijkplayer.utils.StringUtils;
import com.oushangfeng.ijkplayer.utils.ViewUtil;
import com.oushangfeng.ijkplayer.widget.media.IMediaController;
import com.oushangfeng.ijkplayer.widget.media.IjkVideoView;
import com.oushangfeng.ijkplayer.widget.view.MeasureUtil;
...
public class IjkController implements IMediaController, View.OnTouchListener, View.OnClickListener, ProgressView.OnProgressChangeListener {
    ...
    public IjkController(Activity context) {
        mContext = context;
    ...
    public void onProgressChange(float progress) {
        ...
        mPlayer.seekTo((int) (progress) * 1000 - 500);
        mPlayer.pause();

        mTvCurrentTime.setText(StringUtils.generateTime((long) progress * 1000 - 500));
    ...
    private static class IjkControllerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (controller != null) {
                switch (msg.what) {
                    case PLAY_PROGRESS:
                        controller.mTvCurrentTime.setText(StringUtils.generateTime(controller.mPlayer.getCurrentPosition()));

                        // Log.e("TAG","IjkControllerHandler-621行-handleMessage(): "+controller.mPlayer.getBufferPercentage() );

                        controller.mPvPlay.setSaveProgress((controller.mPlayer.getBufferPercentage() + 1) * controller.mPlayer.getDuration() / 1000 / 100.0f);
                        controller.mPvPlay.setProgress(controller.mPlayer.getCurrentPosition() / 1000);

                        Log.e("TAG","IjkControllerHandler-629行-handleMessage(): "+(controller.mPlayer.getCurrentPosition() / 1000)+";"+(controller.mPlayer.getDuration() / 1000) );
                        if (controller.mPlayer.getDuration() > 0 && controller.mPlayer.getCurrentPosition() / 1000 == controller.mPlayer.getDuration() / 1000) {
                            controller.mIvPlay.setActivated(true);
                            Log.e("TAG","IjkControllerHandler-631行-handleMessage(): ");
                            controller.mPvPlay.setProgress(controller.mPlayer.getDuration() / 1000);
                        } else {
                    ...
                    case SHOW:
                        ...
                        controller.mTvCurrentTime.setText(StringUtils.generateTime(controller.mPlayer.getCurrentPosition()));
                        controller.mPvPlay.setMaxProgress(controller.mPlayer.getDuration() / 1000);

                        controller.mTvTotalTime.setText(StringUtils.generateTime(controller.mPlayer.getDuration()));

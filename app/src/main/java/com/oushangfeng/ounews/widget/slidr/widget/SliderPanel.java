...
public class SliderPanel extends FrameLayout {
    ...
    public SliderPanel(Context context, View decorView, View cacheView, SlidrConfig config) {
        super(context);
        mDecorView = decorView;
    ...
    private void init() {
        ...
        LayoutParams flp = new LayoutParams(mScreenWidth / 28, ViewGroup.LayoutParams.MATCH_PARENT);
    ...
    private float mSlidrDistantX, mSlidrDistantY;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                // 优化侧滑的逻辑，不要一有稍微的滑动就被ViewDragHelper拦截掉了，可以在SlidrConfig那里把mSlidrDistantX和mSlidrDistantY作为滑动参数的配置
                if (ev.getX() - mDownX < mSlidrDistantX || Math.abs(mDownY - ev.getY()) > mSlidrDistantY) {
                    return false;
                }
                break;
        }
